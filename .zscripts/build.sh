...
# Prisma resolves the DATABASE_URL relative to prisma/schema.prisma, so the DB
# is created at prisma/db/custom.db — not at db/custom.db (the old location).
DB_SOURCE_PATH="./prisma/db/custom.db"
if [ -f "$DB_SOURCE_PATH" ]; then
    echo "🗄️  复制测试环境数据库到构建产物..."
    mkdir -p "$BUILD_DIR/db"
    cp "$DB_SOURCE_PATH" "$BUILD_DIR/db/custom.db"
    ...
    echo "❌ 未找到测试环境数据库文件 $DB_SOURCE_PATH，无法继续构建生产包"
    exit 1
fi
