...
Task ID: 2
Agent: Main Agent
Task: Fix duplicate sitemap page — audit sitemap architecture, remove obsolete route

Work Log:
- Audited both sitemap files: `src/app/sitemap.ts` (modern Metadata API) vs `src/app/sitemap.xml/route.ts` (obsolete route handler)
- Found `sitemap.xml/route.ts` imports from `src/lib/sitemap-helpers.ts` — which is NOT used by the modern `sitemap.ts`
- Verified `sitemap-helpers.ts` has NO other consumers (grepped all imports)
- Verified the sitemap index generated by the old route pointed to `/sitemap/{id}.xml` chunk routes that DON'T EXIST (broken architecture)
- Deleted `src/app/sitemap.xml/route.ts` and its directory
- Deleted `src/lib/sitemap-helpers.ts` (dead code — only consumer removed)
- Verified: `grep -r sitemap-helpers src/` returns zero results (no dangling imports)
- Verified: `bun run lint` — 0 errors, 0 warnings
- Verified: `/sitemap.xml` returns HTTP 200 with valid `<urlset>` XML from the modern Metadata API sitemap
- Verified: `bun run build` — clean build, 1453 static pages, NO duplicate page warning, NO sitemap warnings

Stage Summary:
- Duplicate sitemap conflict resolved
- Removed 2 dead files: `src/app/sitemap.xml/route.ts` and `src/lib/sitemap-helpers.ts`
- Modern `src/app/sitemap.ts` is the sole sitemap source, served at `/sitemap.xml` via Next.js Metadata API
- Build passes clean with zero sitemap-related warnings
