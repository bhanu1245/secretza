...
export async function GET(request: Request) {
  try {
    ...
    const preview = await previewSingleCityPage(cityId);
    if (!preview) {
    ...
    logError(error, { component: "route:api/admin/seo/generate-city" });
...
export async function POST(request: Request) {
  try {
    ...
    const result = await generateSingleCitySeoPage(cityId);
    if (!result) {
      return NextResponse.json({ error: "City not found" }, { status: 404 });
    }
    ...
    logError(error, { component: "route:api/admin/seo/generate-city" });
    return NextResponse.json({ error: "Failed to generate city SEO page" }, { status: 500 });
  }
}
