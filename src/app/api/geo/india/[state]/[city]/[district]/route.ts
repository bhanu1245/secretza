...
export async function GET(
  _request: Request,
  { params }: { params: Promise<{ state: string; city: string; district: string }> }
) {
  try {
    const { state: stateSlug, city: citySlug, district: districtSlug } = await params;
    ...
    // Fetch localities separately to avoid Prisma relation cache issue
    ...
    logError(error, { component: "route:api/geo/india/[state]/[city]/[district]" });
    return NextResponse.json(
